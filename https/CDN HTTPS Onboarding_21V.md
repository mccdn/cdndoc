...
7. 其他需要向客户了解的信息
	...
	* 源站域名 or 加速域名（自定义域名）作为回源host header？
	* 缓存策略客户有无特殊要求？默认是遵循源站

> **注意**
> 回源host header被源站用来识别是否与源站服务器上配置的域名相同。如果源站默认不能使用自定义加速域名访问时，就需要进行 “回源host header”配置。根据客户自己的情况来配置，一般情况下可以配置成源站域名。如果源站有其他配置，那就遵循源站其他配置。
